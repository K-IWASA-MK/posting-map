...
## 🛑 No Implementation Without Explicit Plan Approval (承認なき実装の絶対禁止と多次元Scope拘束)
AIエージェントは、いかなるコード修正やGit操作を行う際も、事前に `implementation_plan.md` を作成し、ユーザーから明示的な承認（Proceed）を得るまで実行してはならない。

### Scopeの多次元拘束原則 (Multi-Dimensional Scope Binding)
1. **「Scope内」の厳格な再定義**:
   - 「Scope内」とは単なる「許可されたファイルパス一覧」ではない。**「ユーザーから指示され承認された【タスク目的（Purpose）】および【許可された変更内容（Allowed Modifications）】のみ」**を指す。
   - たとえ対象ファイル（例: `index.html`）内に含まれるコードであっても、ユーザーから指示されていない新規UI、新規ブレークポイント、メディアクエリ（`@media`）、イベントリスナー、自発的レスポンシブ化・最適化は**重大なScope違反（即時REJECT・HARD STOP）**とする。
2. **`implementation_plan.md` への3大拘束項目の記載義務**:
   - **タスク目的（Purpose）**: 何のための変更か（何を解決するか）。
   - **許可される変更内容（Allowed Modifications）**: どの行・どの要素をどのように変更するか。
   - **禁止・対象外の境界（Forbidden / Out-of-Scope Boundaries）**: 指示されていない変更（未指示のモバイル最適化、未指示の画面対応、未指示のCSS調整等）を明示的に禁止境界として定義する。
...
3. **未指示機能・自発的最適化の追加禁止**: ユーザー指示にない新規UI、新規ブレークポイント、メディアクエリ、イベントリスナー、自発的最適化（モバイル対応等）をコードに追加してはならない。
...
- Scope外の変更が必要になった場合
- **タスク目的外の問題・違和感を発見した場合（モグラ叩き・自発パッチの絶対禁止）**: 検証中に、承認されたタスク目的とは直接関係のない不具合（例: 「スマホで横揺れする」「文字が溢れる」「別解像度で崩れる」「既存の未対応箇所の発見」等）を発見した場合、自己判断で修正コードを追加することは絶対禁止。その瞬間に即時 HARD STOP し、ユーザーへ報告して指示を仰ぐこと。
...
- **問題発生時の原則**: 修正可能な問題は「**今回の承認された変更によって直接引き起こされた問題（実装ミスやデグレ）のみ**」に厳格に限定される。元から存在していた問題や新しく目についた未対応領域の不備はすべてScope外であり、勝手に対応・修正せず直ちに作業をHARD STOPして報告する。
...
- **自発的パッチ・連鎖修正の禁止**: 検証中にタスク目的外の不具合や改善点を発見しても、勝手にパッチを投入してはならない。即座にHARD STOPしてユーザーへ報告する。
...
- **Handover Package（受領仕様）**: Developerから「①ユーザー指示原文（Instruction Prompt）および承認された目的・変更許可境界、②タスク/Scope、③変更ファイル一覧、④差分（Git Diff）、⑤客観的Evidence」を受領して独立査読する。

#### 担当範囲
- 最上位絶対原則（地区非依存・コピー原則）の遵守を監視（`active/` への地区固有情報・ハードコードの混入検知）。
- スコープ厳守および余計な差分（ユーザー指示にない新規UI・ブレークポイント・メディアクエリ・イベントリスナー・自発的最適化、リファクタリング、フォーマット変更、コメント残置）の完全排除。
...
- 以下の状態である場合、コミットおよび完了報告を**却下（Reject）**し、Developerへ差し戻す（自分では直さない）。
  ...
  - ユーザー指示および承認Planにない新規UI、新規ブレークポイント、メディアクエリ、イベントリスナー、自発的最適化が含まれている（自発的仕様拡張 / Intent-Diff不一致）。
...
7. **修正（承認された変更に起因するデグレのみ）**: 今回の承認された変更によって直接引き起こされた問題（実装ミスやデグレ）のみ修正可能。元から存在していた問題や新しく目についた未対応領域の不備はScope外であり、修正せず直ちにHARD STOPすること。
8. **再検証**: 修正後、再度ローカル検証を回す（承認された変更範囲内でPASSするまで6〜8を繰り返す）。
