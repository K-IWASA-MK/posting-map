...
```
> ✅ 12シート完全性、CSV件数一致、SSOT一致、進捗0%初期化、端末管理初期枠、前地区データ混入ゼロがすべて自動判定されます。

---

### Step 10: Dashboard 端末登録・動作確認
1. 対象地区の管理コックピット URL（例: `https://<新地区ドメイン>/manager/` またはローカル）を開きます。
2. 最初のPCが **PC-01** として自動バインドされ、コックピット画面が表示されます。
