...
## 🏛️ メインステージUI原則（Main Stage UI Constitution）

「外枠は固定、選択画面をメイン化、データ量だけ内部スクロール。地図だけは空間UIとして固定。」

### 左ナビ・メインビュー対応原則
| 左ナビ項目 | メインの役割 | 表示方式 |
|---|---|---|
| 🗺️ **配布エリア** | 地図・空間把握 | **地図領域最大化・固定・パン/ズーム優先** |
| 📝 **配布実績** | 配布員ランキング | **タイトル固定 ＋ データカード領域内部縦スクロール** |
| 📦 **保有チラシ** | 拠点別保有状況 | **タイトル固定 ＋ データカード領域内部縦スクロール** |
| 📋 **名簿** | 登録配布員一覧 | **タイトル固定 ＋ データカード領域内部縦スクロール** |
| 🤝 **受渡要請** | チラシ受渡要請一覧 | **タイトル固定 ＋ データカード領域内部縦スクロール** |

### 4項目（実績・在庫・名簿・受渡）に共通する構造
1. **メインステージ展開**: 選択した項目がメインステージいっぱいに展開する。
2. **外枠完全固定**: メインステージの外枠・境界線は完全固定。
3. **タイトル上部固定**: 見出しタイトルバッジ（例: `🏆 配布実績 ランキング一覧`）は常に上部に固定。
4. **内部スクロール**:
   - 件数が少なければ、そのまま上から整然と表示。
   - 件数が増えて画面に収まらなくなったら、**データカード領域（`#main-stage-*-content`）の内側だけを縦スクロール**させる。
   - 画面全体（`body` / `main`）は絶対にスクロールさせない。
5. **周辺計器の絶対固定**:
   - `KPI HEADER` は絶対固定。
   - `左ナビ` は絶対固定。
   - `LIVE BAR` は絶対固定。
...
### 設計哲学
- **横長カードの正当性**: 横長カード自体は、管理者が1行で「順位・ID・氏名・状態・実績」を一目で直感把握するための正しい情報単位である。
- **防止すべき悪態**: カードが増えた結果として「メイン画面の外枠そのものが縦に伸びること」を絶対に防止する。
- **地図の空間UI性**: 🗺️ 配布エリアだけはカード一覧ではなく「空間UI」であるため、地図領域そのものを最大化し、スクロールではなく地図操作（パン/ズーム/クリック）を最優先とする。
