...
## DATA STRUCTURE VERIFICATION RULE (データ構造事前確認規約)

実装前に対象データソースの実構造を確認すること。
...
### 🚫 禁止事項
- データ構造を確認せずに実装を開始すること
- 「〜だろう」という推測に基づいてデータアクセスパターンを決定すること
- 実データを確認せずにログ追記型 / 固定マスター型を判断すること

### ⚙️ 必須手順
1. 対象データソースの実データを取得・確認する
2. データ構造（型・形式・更新方式）を確定する
3. 確定した構造に基づいて実装する
